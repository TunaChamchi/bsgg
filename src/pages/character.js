...
import { getCharacter, addJson, getSeason, getSeasonString } from 'lib/data'
...
class Character extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            matchMode: getSeason(), 
            teamMode: 0,
            matchingMode: ['전체', '시즌1', '프리시즌2', '시즌2', '프리시즌3', '시즌3'],
    ...
    fetchHandler = async (query, prevState) => {
        ...
        if (userName !== this.state.userName
            ...
            let maxMmr = 0;
            if (_userStat['seasonStats'][getSeasonString()]) {
                Object.keys(_userStat['seasonStats'][getSeasonString()]).forEach(t => 
                    maxMmr = Math.max(maxMmr, _userStat['seasonStats'][getSeasonString()][t]['mmr'])
                )
            }
            ...
            <div className={"record_cha0_tab"+(matchMode===idx?' actived':'')}  key={'record_cha0_tab_'+idx}
                onClick={(e) => this.setState({ matchMode:idx })}>
                {
                    idx === 0 ?
                        ...
                        :
                        idx % 2 == 1 ?
                            intl.formatMessage({id: 'season'}) + (idx/2 + 0.5)
                            :
                            intl.formatMessage({id: 'freeseason'}) + (idx/2 + 1)
                }
            </div>
        )
    }
